...
async function parseMatchRows(account: AccountContext, rows: CsvRow[], createOpponentSchools: boolean, enforceSavedFormat: boolean) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (enforceSavedFormat && !seasonFormat.eventOrder.includes(positionText)) {
      throw new Error(
...
export async function previewMeet(account: AccountContext, rows: CsvRow[]) {
  await ensureSchema();
  await ensureCurrentRatingModel(account.id);
  const parsed = await parseMatchRows(account, rows, false, true);
...
export async function importMatches(account: AccountContext, rows: CsvRow[]) {
  await ensureSchema();
  const parsed = await parseMatchRows(account, rows, true, false);
...
async function recomputeRatings(accountId: string) {
  ...
  for (const [id, group] of historyGroups) {
    ...
    const projectedWins = samples.reduce(
      (total, sample) => total + eloWinProbability(sample.homeElo, sample.opponentElo + offset),
      0,
    );
    calibrations.push({
      id,
